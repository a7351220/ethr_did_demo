...
export class AlchemyProvider extends ethers.providers.JsonRpcProvider {
...
async function checkOwnership(ethrDid: EthrDID, address: string): Promise<boolean> {
  try {
...
export async function registerDID(did: string, privateKey: string): Promise<string> {
  ...
  // 检查身份所有权
  console.log('Checking DID ownership...');
  const isOwner = await checkOwnership(ethrDid, wallet.address);
  if (!isOwner) {
      throw new Error('The provided private key does not own this DID');
  }
