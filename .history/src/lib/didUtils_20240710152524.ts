...
export async function registerDID(did: string, privateKey: string): Promise<string> {
    ...
    if (didParts.length !== 4 || didParts[0] !== 'did' || didParts[1] !== 'ethr' || didParts[2] !== 'sepolia') {
      throw new Error('Invalid DID format. Expected: did:ethr:sepolia:publicKey');
    }
    const publicKeyFromDid = didParts[3];
    ...
    // 从私钥计算公钥
    const publicKeyFromPrivate = ethers.computeAddress(privateKey).slice(2);
  
    // 验证公钥匹配
    if (publicKeyFromDid.toLowerCase() !== publicKeyFromPrivate.toLowerCase()) {
      throw new Error('Private key does not match the public key in the DID');
    }
  
    // 从公钥计算以太坊地址
    const address = ethers.computeAddress('0x' + publicKeyFromDid);
    ...
    try {
      ...
      const tx = await didReg.setAttribute(
        ...
        ethers.getBytes('0x' + publicKeyFromDid),
        86400 // 有效期为1天
      );
