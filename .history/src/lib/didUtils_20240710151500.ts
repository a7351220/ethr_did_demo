...
export async function registerDID(did: string, privateKey: string): Promise<string> {
    ...
    if (didParts.length < 3 || didParts[0] !== 'did' || didParts[1] !== 'ethr') {
      throw new Error('Invalid DID format. Expected: did:ethr:[network:]identifier');
    }
    const identifier = didParts[didParts.length - 1];
    ...
    // 验证公钥或地址匹配
    if (identifier.toLowerCase() !== publicKey.slice(2).toLowerCase() &&
        identifier.toLowerCase() !== compressedPublicKey.slice(2).toLowerCase() &&
        identifier.toLowerCase() !== wallet.address.toLowerCase()) {
      throw new Error('Private key does not match the identifier in the DID');
    }
  
    // 确定要使用的地址
    const address = ethers.utils.isAddress(identifier) ? identifier : wallet.address;
