...
function initProvider(): ethers.Provider {
  if (!provider) {
    provider = new ethers.InfuraProvider(providerConfig.network, {
      projectId: providerConfig.projectId,
      projectSecret: providerConfig.projectSecret
    });
    console.log('Provider initialized:', (provider as any).network.name);
  }
  return provider;
}
