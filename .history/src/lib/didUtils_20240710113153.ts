...
export async function getDIDAttributes(did: string): Promise<Record<string, any>> {
 ...
 /**
 * 为 DID 添加一个委托
 * @param did - DID（格式：did:ethr:address）
 * @param privateKey - DID 控制者的私钥
 * @param delegateAddress - 委托地址
 * @param delegateType - 委托类型（例如：'veriKey'）
...
export async function addDelegate(
    ...
    try {
      ...
      const tx = await didReg.addDelegate(
        address,
        ethers.getBytes(delegateType),
        delegateAddress,
        expiresIn
      );
  
      console.log('Delegate addition transaction sent, hash:', tx.hash);
  ...
  export async function setAttribute(
    ...
    try {
      ...
      const txHash = await ethrDid.setAttribute(key, value, expiresIn);
      console.log('Attribute set, transaction hash:', txHash);
